...
const useFetchWalletTransactions = () => {
  ...
  const {
    ...
    isError,
    error,
  } = useQuery<baseResponseWithPagination<WalletTransaction[]>>({
    queryKey: [
      "walletTransactions",
      currentPageParam,
      limitParam,
  ...
  const currentPage = walletTransactions?.meta?.current_page || 1;
  const totalPages = walletTransactions?.meta?.total_pages || 1;
  const pageCollections = walletTransactions?.meta?.page_collections || [];
