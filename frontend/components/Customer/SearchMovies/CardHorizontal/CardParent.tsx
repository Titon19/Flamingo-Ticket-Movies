...
const CardParent = ({
  ...
  return (
    <div className="flex flex-col gap-3 m-4" onClick={onClick}>
      <h1 className="text-md font-bold">{amount} Items Found</h1>
      {children}
    </div>
  );
};
...
const Card = ({
  ...
  return (
    <Link
      href={href || "#"}
      className="flex gap-3 justify-between items-center border-2 border-neutral-800 rounded-xl p-1 hover:border-pink-600 transition-colors ease-in-out duration-300"
    >
      {children}
    </Link>
  );
