...
const CardParent = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="flex flex-col gap-3">
      <h1 className="text-md md:text-lg lg:text-xl font-bold">Choosed Movie</h1>
      {children}
    </div>
  );
};

const Card = ({ children }: { children: React.ReactNode; href?: string }) => {
  return (
    <div className="flex gap-3 justify-between items-center border-2 border-pink-600 rounded-xl p-1 transition-colors ease-in-out duration-300">
      {children}
    </div>
  );
};
